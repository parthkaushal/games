...
gameScene.create = function() {
  ...
  for(let i = 0; i < items.length; i++) {
    ...
    item.on('pointerdown', function(pointer) {
      ...
      let result = this.processAnswer(this.words[i].kannada);
...
gameScene.showNextQuestion = function(){
  ...
  // show the text of the word in kannada
  this.wordText.setText(this.nextWord.kannada);
};
...
gameScene.processAnswer = function(userResponse) {
  ...
  if(userResponse == this.nextWord.kannada) {
    // it's correct
